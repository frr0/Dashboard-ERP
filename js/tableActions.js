  ...
  addEventListener('click',e=>{
    ...
    if(t.classList.contains('btn-elimina')){
      if(confirm('Vuoi davvero eliminare questa riga?')){
        const tr=t.closest('tr'),tb=tr&&tr.closest('table');
        if(tr&&tb&&tb._erpState){
          const s=tb._erpState,g=parseInt(tr.dataset.globalIndex||'-1',10);
          if(!isNaN(g)){
            if(s.filteredItems){const r=(s.filteredItems??[])[g-(s.page-1)*s.pageSize],i=s.items.indexOf(r);if(i>=0)s.items.splice(i,1);s.filteredItems=s.filteredItems.filter(x=>x!==r);}else s.items.splice(g,1);}
          const tot=(s.filteredItems??s.items).length,p=Math.max(1,Math.ceil(tot/s.pageSize));if(s.page>p)s.page=p;
          if(typeof window.__erpRenderPage==='function')window.__erpRenderPage(tb);
        }else if(tr)tr.remove();
      }return;
    }
    if(t.classList.contains('btn-modifica')){
      rigaCorrente=t.closest('tr');if(!rigaCorrente)return;
      const celle=rigaCorrente.querySelectorAll('td:not(.azioni)'),tb=rigaCorrente.closest('table'),ths=tb?tb.querySelectorAll('thead th'):[],popup=document.getElementById('popup-contenuto');
      let html='';
      celle.forEach((td,i)=>{const v=td.textContent,l=ths[i]?ths[i].textContent:`Campo ${i+1}`;html+=`<div class=\"mb-3\"><label for=\"input-${i}\" class=\"form-label\">${l}</label><input type=\"text\" id=\"input-${i}\" class=\"form-control\" data-index=\"${i}\" value=\"${v}\"></div>`;});
      if(popup)popup.innerHTML=html;window.myModal.show();return;
    }
  });
};
